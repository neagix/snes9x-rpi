/***********************************************************************************
  ...
  snes9x-rpi repository: https://github.com/neagix/snes9x-rpi
...
void S9xParseDisplayArg(char **argv, int &i, int argc) {
    ...
    if (!strncasecmp(argv[i], "-v", 2)) {
        switch (argv[i][2]) {
            case '1': GUI.video_mode = VIDEOMODE_BLOCKY;
                break;
            case '2': GUI.video_mode = VIDEOMODE_TV;
                break;
            case '3': GUI.video_mode = VIDEOMODE_SMOOTH;
                break;
            case '4': GUI.video_mode = VIDEOMODE_SUPEREAGLE;
                break;
            case '5': GUI.video_mode = VIDEOMODE_2XSAI;
                break;
            case '6': GUI.video_mode = VIDEOMODE_SUPER2XSAI;
                break;
            case '7': GUI.video_mode = VIDEOMODE_EPX;
                break;
            case '8': GUI.video_mode = VIDEOMODE_HQ2X;
                break;
        }
    } else
        S9xUsage();
...
bool S9xInitDisplay(int argc, char **argv) {
    /*
     * domaemon
     *
    ...
    // initialize Raspberry Pi dispmanx
    GUI.rpi2D = new Raspberry2D(SNES_WIDTH, SNES_HEIGHT_EXTENDED + 4);
    
    // vertical ratio
    float ratio = ((float)GUI.rpi2D->DisplayHeight) / GUI.rpi2D->Height;
    
    float finalW = GUI.rpi2D->Width * ratio;

    // attach surface
    if (!GUI.rpi2D->Attach(finalW, GUI.rpi2D->DisplayHeight)) {
        delete GUI.rpi2D;
    ...
    // create the texture
    GUI.textureBuffer = new Texture2D(GUI.rpi2D->Width, GUI.rpi2D->Height, 2, GL_RGB, GL_UNSIGNED_SHORT_5_6_5);
...
void S9xPutImage(int w, int h) {
    ...
    GUI.textureBuffer->Render(GUI.snes_buffer);
    
    GUI.rpi2D->Draw(*GUI.textureBuffer);
    
    GUI.rpi2D->SwapBuffers();
