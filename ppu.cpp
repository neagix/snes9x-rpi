...
extern int S9xConfiguredJoypads;
extern volatile uint32 DirectJoypads[5];

//S9xReadJoypad(i) is just a macro because we have buffered volatile DirectJoypads
#define S9xReadJoypad(jpIdx)    DirectJoypads[jpIdx]

void S9xUpdateJoypads(struct InternalPPU *ippu) {

    // neagix: the input thread is taking care of updating these

    for (int i = 0; i < S9xConfiguredJoypads; i++) {
        ippu->Joypads[i] = S9xReadJoypad(i);
    }

    // neagix: why were these key combos prevented?
    /*
    ...
    if (SNESGameFixes.TouhaidenControllerFix
            && (ippu->Controller == SNES_JOYPAD
            || ippu->Controller == SNES_MULTIPLAYER5)) {
        for (i = 0; i < S9xConfiguredJoypads; i++) {
            if (ippu->Joypads[i])
                ippu->Joypads[i] |= 0xffff0000;
        }
    }

    // neagix: this should be supported through analog sticks
