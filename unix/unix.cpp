...
//    Settings.DisplayFrameRate = FALSE;
    ...
    // we use auto framerate because it goes too fast otherwise
...
/*#ifdef PANDORA
    ...
    // neagix: check that this does not cause troubles
...
void InitTimer() {
    ...
    // neagix: even if thread id disabled, mutex might still be used
#ifdef USE_THREADS
    if (Settings.ThreadSound) {
        pthread_mutex_init(&mutex, NULL);
...
// neagix: set to true when sound is being generated by CPU call (sync sound)
//static volatile bool8 block_signal = FALSE;

// used to block sound generation while playing on sound device
...
static inline bool SND_TRYLOCK(const char *context) {
    //#ifdef USE_THREADS
    //    if (Settings.ThreadSound) {    
...
// neagix: this is called by CPU only when sound sync is enabled
...
void S9xGenerateSound() {
    ...
    if (!SND_TRYLOCK("S9xGenerateSound"))
        return;
