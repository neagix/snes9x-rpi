...
static inline int S9xGetEnvelopeHeight(int channel) {
    // neagix: currently disabled, it was there in 1.51, need to investigate if still needed in this fork of 1.53
...
void S9xAPUWritePort(int port, uint8 byte) {
    ...
    //	APU.OutPorts[port] = byte;
    // neagix: this is grey
    Memory.FillRAM [port] = byte;
    IAPU.RAM [(port & 3) + 0xf4] = byte;
}
...
void S9xResetAPU() {
    ...
    // neagix: disabled, we always use 0xFF (some games require 0x00)
    //memset (IAPU.RAM, SNESGameFixes.APURAMInitialValue, 0x10000);
    ...
    memset(APU.OutPorts, 4, sizeof (uint8_t));
    ...
    // neagix: this is no more used in favor of 'Timings'
...
static void PerformSoftReset(uint8 byte) {
    APU.DSP [APU_FLG] = APU_MUTE | APU_ECHO_DISABLED | (byte & 0x1f);
    APU.DSP [APU_ENDX] = 0;
    APU.DSP [APU_KOFF] = 0;
    APU.DSP [APU_KON] = 0;
    S9xSetEchoWriteEnable(FALSE);

    // neagix: could be used for debugging
    ...
    switch (reg) {
        ...
        case APU_P_HIGH + 0x70:
            ...
            S9xSetSoundHertz(reg >> 4,
                    ((((byte << 8) + APU.DSP [reg - 1]) & FREQUENCY_MASK) * 32000) >> 12);
            break;
...
//    if (APU. >= APU_MINIMUM_SAMPLE_BLOCK /*|| !spc::sound_in_sync*/)
//        S9xLandSamples();
}
